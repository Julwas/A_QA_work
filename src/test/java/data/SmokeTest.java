...
public class SmokeTest {
    ...
    public void calculatorIKTTest(){
        driver.get("https://clinic-cvetkov.ru/company/kalkulyator-imt/");// переход к н странице
        System.out.println(driver.getCurrentUrl());
        System.out.println(driver.getTitle());
        //System.out.println(driver.getPageSource());//html дом модель

        WebElement weightInput = driver.findElement(By.name("weight"));
        //findElement add->exaption, two & more locator-> return 1j locator
        // findElements ->return colection
        WebElement heightInput = driver.findElement(By.name("height"));
        WebElement calcButton = driver.findElement(By.id("calc-mass-c"));

        List<WebElement> buttonsList = driver.findElements(By.tagName("button"));//найти в кол. елементы по id
        System.out.println(buttonsList.size());//кол. найденных елементов
    ...
    public void validateSKFTest() throws InterruptedException {
        ...
        WebElement selectWebElement = driver.findElement(By.id("oSex"));//находим элемент
        Select selectSex = new Select(selectWebElement);//создать селект элемент на основе найенного элемента
//select->что бы имет 3 способа поиска
        selectSex.selectByIndex(1);
        sleep(2000);

        selectSex.selectByValue("0"); //выбор по атрибуту Value
        sleep(2000);

        selectSex.selectByVisibleText("женский");// по значению которое отображается
        sleep(2000);
